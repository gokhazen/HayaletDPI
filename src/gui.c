...
void ShowTab(HWND hwndDlg, int index) {
    ...
    // Tab 4 - Update
    int t4[] = { IDC_GRP_UPDATE, IDC_UPDATE_TITLE, IDC_UPDATE_DESC, IDC_BTN_CHECK_UPDATE };
    // Tab 5 - About
    int t5[] = {
        IDC_GRP_ADVANCED,
...
INT_PTR CALLBACK SettingsDialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_INITDIALOG: {
            ...
            tie.pszText = "Update"; TabCtrl_InsertItem(hTab, 3, &tie);
            tie.pszText = "About"; TabCtrl_InsertItem(hTab, 4, &tie);
            ShowTab(hwndDlg, 0);
        ...
        case WM_COMMAND: {
            ...
            if (id == IDC_BTN_CHECK_UPDATE) CreateThread(NULL, 0, CheckForUpdates, (LPVOID)1, 0, NULL);
...
DWORD WINAPI CheckForUpdates(LPVOID lpParam) {
    HINTERNET hInternet = InternetOpen("HayaletDPI-Updater", INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, 0);
    if (!hInternet) return 0;

    HINTERNET hUrl = InternetOpenUrl(hInternet, "https://raw.githubusercontent.com/gokhazen/HayaletDPI/refs/heads/main/VERSION", NULL, 0, INTERNET_FLAG_RELOAD | INTERNET_FLAG_NO_CACHE_WRITE, 0);
    if (!hUrl) { InternetCloseHandle(hInternet); return 0; }

    char remoteVersion[64] = {0};
    DWORD bytesRead = 0;
    if (InternetReadFile(hUrl, remoteVersion, 63, &bytesRead)) {
        remoteVersion[bytesRead] = '\0';
        
        // Precise Clean: Remove all whitespace and non-printable characters
        char cleanRemote[64] = {0};
        int j = 0;
        for (int i = 0; i < (int)bytesRead; i++) {
            if ((remoteVersion[i] >= '0' && remoteVersion[i] <= '9') || remoteVersion[i] == '.') {
                cleanRemote[j++] = remoteVersion[i];
            } else if (j > 0) break; // Stop after the first numeric block
        }
        cleanRemote[j] = '\0';
